...
export function StudentProfileFields({value,onChange,errors={},disabled=false}:{value:StudentProfileDraft;onChange:(value:StudentProfileDraft)=>void;errors?:StudentProfileErrors;disabled?:boolean}) {
  return <>
    <Input label="ID студента" autoComplete="nickname" required maxLength={33} value={value.username} disabled={disabled} onChange={event=>onChange({...value,username:event.target.value})} error={errors.username} placeholder="@your_name" hint="Уникальное имя в TaskRank: от 3 до 32 латинских букв, цифр или _." />
