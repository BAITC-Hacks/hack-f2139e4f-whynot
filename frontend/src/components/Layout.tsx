...
const navItems = {
  business: [
    ...
    { to: '/business/history', label: 'История задач', icon: History },
  ],
  student: [
    { to: '/catalog', label: 'Каталог задач', icon: LayoutGrid },
...
export function Layout() {
  ...
  const visibleNav = actor ? navItems[role] : [navItems.business[0]];
  const authLabels: Record<string, string> = { '/login': 'Вход', '/register': 'Регистрация', '/forgot-password': 'Восстановление пароля', '/reset-password': 'Новый пароль' };
  const activePage = authLabels[location.pathname] ?? visibleNav.find(item => item.to === location.pathname)?.label ?? (location.pathname.includes('proposals') ? 'Отклики команд' : location.pathname.includes('edit') ? 'Редактор задачи' : 'Карточка задачи');
  ...
  const signOut = async () => { if (loggingOut) return; setLoggingOut(true); try { await logout(); window.location.replace('/catalog'); } catch (caught) { toast(errorMessage(caught), 'error'); } finally { setLoggingOut(false); } };
  ...
  return <div className="app-shell"><a href="#main-content" className="skip-link">К содержимому</a>
    <header className="app-topbar"><div className="topbar-brand-group"><button ref={menuButtonRef} type="button" className="icon-button mobile-menu-button" aria-label={menuOpen ? 'Закрыть меню' : 'Открыть меню'} aria-expanded={menuOpen} aria-controls="app-sidebar" onClick={() => setMenuOpen(value => !value)}>{menuOpen ? <X size={21} /> : <Menu size={21} />}</button><Link to="/catalog" className="brand" aria-label="TaskRank — каталог задач"><span className="brand-mark"><Layers3 size={21} strokeWidth={2.2} /></span><span>Task<span className="brand-accent">Rank</span><span className="brand-dot">.</span></span></Link><span className="topbar-separator" /><span className="brand-caption">Место, где идеи растут</span></div>
      <div className="topbar-right">{USE_MOCKS && <div className="role-switch" aria-label="Роль в демонстрации"><button type="button" className={role === 'business' ? 'active' : ''} aria-pressed={role === 'business'} onClick={() => chooseRole('business')} disabled={loading || !actors.some(item => item.role === 'business')}><BriefcaseBusiness size={15} />Бизнес</button><button type="button" className={role === 'student' ? 'active' : ''} aria-pressed={role === 'student'} onClick={() => chooseRole('student')} disabled={loading || !actors.some(item => item.role === 'student')}><GraduationCap size={17} />Команда</button></div>}{actor ? <><div className="account"><span className="avatar">{initials}</span><div><strong>{actor.name}</strong><small>{role === 'business' ? 'Представитель бизнеса' : team?.name ?? 'Студент'}</small></div></div>{!USE_MOCKS && <Button variant="ghost" size="sm" onClick={() => { void signOut(); }} loading={loggingOut}><LogOut size={16} /> Выйти</Button>}</> : !loading && <><Link className="button button-secondary button-sm" to="/login">Войти</Link><Link className="button button-primary button-sm" to="/register">Регистрация</Link></>}</div>
    </header>
    ...
    <aside ref={sidebarRef} id="app-sidebar" className={`app-sidebar ${menuOpen ? 'is-open' : ''}`}><div className="sidebar-section"><span className="nav-label">РАБОЧЕЕ ПРОСТРАНСТВО</span><nav className="main-nav" aria-label="Главная навигация">{visibleNav.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}><Icon size={18} strokeWidth={1.8} /><span>{label}</span>{to === '/team/progress' && <span className="nav-dot" />}</NavLink>)}</nav></div>
      {USE_MOCKS && <div className="actor-picker"><label htmlFor="demo-actor">Демо-профиль</label><select id="demo-actor" value={actor?.id ?? ''} disabled={loading || actors.length === 0} onChange={event => { selectActor(event.target.value); navigate('/catalog'); }}>{!actor && <option value="">Выберите профиль</option>}{actors.filter(item => item.role === role).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><span className="actor-hint">Данные сохраняются до перезагрузки</span></div>}
      ...
      <div className="sidebar-quest"><span className="quest-label">{role === 'business' ? <Sparkles size={14} /> : <Flag size={14} />}{role === 'business' ? 'ОТ ИДЕИ К РЕЗУЛЬТАТУ' : 'СЛЕДУЮЩИЙ ШАГ'}</span><h3>{!actor ? 'Присоединяйтесь к TaskRank' : role === 'business' ? 'Начните с потребности' : 'Ваш опыт нужен бизнесу'}</h3><p>{!actor ? 'Создайте аккаунт бизнеса или студента, чтобы работать над задачами и сохранять результаты.' : role === 'business' ? 'Расскажите о задаче — поможем превратить её в понятную карточку.' : 'Найдите задачу, предложите решение и получите опыт за результат.'}</p><Link to={!actor ? '/register' : role === 'business' ? '/new' : '/recommendations'}>{!actor ? 'Создать аккаунт' : role === 'business' ? 'Создать задачу' : 'Найти свой проект'}<ArrowRight size={14} /></Link><Sparkles className="quest-decoration" size={64} strokeWidth={.8} aria-hidden="true" /></div>
    ...
    <main id="main-content" className="app-main" tabIndex={-1}><div className="breadcrumb"><span>Рабочее пространство</span><ChevronRight size={12} /><span>{activePage}</span></div>{error && <div className="page"><ErrorBanner message={error} onRetry={retry} /></div>}{loading ? <div className="page"><Skeleton lines={3} /></div> : <Outlet />}<footer className="main-footer"><span>TaskRank — от идеи до первого результата</span><span><CircleHelp size={13} />Решения принимают люди</span></footer></main>
  </div>;
}
