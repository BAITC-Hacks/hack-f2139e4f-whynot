...
export function TaskDetailPage() {
  ...
  return <div className="page">
    ...
    {task.loading ? <Skeleton lines={9} /> : task.data && <>
      <PageHeader title={task.data.card.title} subtitle={<span className="row"><Badge>{topicLabel(task.data.card.topic)}</Badge><LevelBadge level={task.data.rating.readiness} /><span>Опубликовано {formatDate(task.data.published_at)}</span></span>} actions={actor && role === 'business' ? <Link className="button button-secondary" to={`/tasks/${id}/proposals`}>Отклики <ArrowRight size={16} /></Link> : undefined} />
