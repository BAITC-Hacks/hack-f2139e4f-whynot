...
export function TaskDetailPage() {
  const { id = '' } = useParams();
  const { actor, role, team,teamLoading,refreshTeam } = useRole();
  useEffect(()=>{if(actor?.role==='student')void refreshTeam().catch(()=>{})},[actor?.id,refreshTeam]);
  ...
  const ready=team?.ready??USE_MOCKS;

  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!actor || !team || busy || !leader || !ready || teamLoading) return;
  ...
  return <div className="page">
    ...
    {task.loading ? <Skeleton lines={9} /> : task.data && <>
      <PageHeader title={task.data.card.title} subtitle={<span className="row"><Badge>{topicLabel(task.data.card.topic)}</Badge><LevelBadge level={task.data.rating.readiness} /><span>Опубликовано {formatDate(task.data.published_at)}</span></span>} />
      {!actor && <div className="notice"><p><Link to="/login" state={{ from: `/tasks/${id}` }}>Войдите</Link> или <Link to="/register" state={{ from: `/tasks/${id}` }}>зарегистрируйтесь</Link> как студент, чтобы предложить решение и сохранить результаты.</p></div>}
      <div className="split-layout detail-layout"><div className="stack">
        ...
        {role === 'student' && <Card id="proposal"><h2 className="section-title"><Send size={20} /> Ваше предложение</h2>{teamLoading ? <Skeleton lines={3}/> : !team ? <div className="stack"><p className="muted">Создайте команду или присоединитесь по приглашению. Для отклика нужно 3–5 участников.</p><Link className="button button-primary" to="/team/profile">Создать или найти команду <ArrowRight size={16} /></Link></div> : proposals.loading ? <Skeleton lines={3} /> : proposals.error ? <ErrorBanner message={proposals.error} onRetry={proposals.reload} /> : existing.length ? <div className="stack">{existing.map(proposal => <div className="notice" key={proposal.id}><CheckCircle2 size={21} /><div><strong>Вы уже отправили предложение</strong><p>{proposalLabels[proposal.status]}</p>{proposal.decision_note && <p>{proposal.decision_note}</p>}</div></div>)}<Link className="button button-secondary" to="/team/proposals">Посмотреть мои отклики <ArrowRight size={16} /></Link></div> : !ready || !leader ? <div className="stack"><p className="notice">{!ready ? 'Для отклика соберите команду из 3–5 участников. Участников приглашает лидер.' : 'Отклик отправляет лидер команды. После отправки все участники смогут обсудить задачу с бизнесом.'}</p><Link className="button button-secondary" to="/team/profile">Моя команда</Link></div> : <form className="stack" onSubmit={submit} noValidate><p className="muted">Расскажите, как ваша команда решит задачу. Бизнес рассмотрит каждое предложение самостоятельно.</p><Textarea label="Идея решения" required maxLength={8000} counter value={form.idea} onChange={event => setForm({ ...form, idea: event.target.value })} error={fieldErrors.idea} placeholder="Что вы предлагаете и почему это поможет бизнесу?" /><Textarea label="План работы" required maxLength={8000} counter value={form.plan} onChange={event => setForm({ ...form, plan: event.target.value })} error={fieldErrors.plan} placeholder="Опишите основные шаги и результат каждого из них" /><Input label="Срок и этапы" required maxLength={500} value={form.timeline} onChange={event => setForm({ ...form, timeline: event.target.value })} error={fieldErrors.timeline} placeholder="Например: прототип за 2 недели, пилот — за месяц" /><Input label="Ссылка на прототип или портфолио" type="url" maxLength={2000} value={form.prototype_url || ''} onChange={event => setForm({ ...form, prototype_url: event.target.value })} error={fieldErrors.prototype_url} placeholder="https://… — необязательно" />{submitError && <ErrorBanner message={submitError} />}<div className="form-actions"><span className="muted text-small">От команды «{team.name}»</span><Button type="submit" loading={busy}><Send size={16} /> Отправить предложение</Button></div></form>}</Card>}
