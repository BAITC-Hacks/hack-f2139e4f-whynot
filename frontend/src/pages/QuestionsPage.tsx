...
import { appendQuestionHistory, cardLengthErrors, fieldLimit, filledCardAnswers, mergeRoundAnswers } from '../ui/assistRounds';
import type { AssistResult, CardData, CardField, Question, Task } from '../types';
...
function QuestionsScreen() {
  ...
  const [questionHistory, setQuestionHistory] = useState<Question[]>(() => appendQuestionHistory([], initial?.assist?.questions ?? []));
    ...
    void (async () => {
      try {
        ...
        if (active.current && id === request.current) {
          setAssist(result);
          setQuestionHistory(appendQuestionHistory([], result.questions));
        }
      } catch (reason) {
  ...
  const candidate: CardData | null = assist ? mergeRoundAnswers(assist.suggested_card, answers) : null;
  const fieldErrors = candidate ? cardLengthErrors(candidate) : {};
  const invalidCard = Object.keys(fieldErrors).length > 0;

  async function regenerate() {
    if (!actor || !taskId || locked.current) return;
    if (invalidCard) { setError(Object.values(fieldErrors).join(' ')); return; }
    locked.current = true;
    setBusy('assist');
    setError(null);
    try {
      const result = await api.assistTask(actor.id, taskId, candidate ? filledCardAnswers(candidate) : {}, questionHistory);
      if (active.current) {
        setAssist(result);
        setQuestionHistory(current => appendQuestionHistory(current, result.questions));
        setAnswers({});
        setConflict(false);
        setServerTask(null);
      }
  ...
  return <div className="page">
    ...
    <div className="editor-layout">
      <div className="stack">
        {assist && <>
          <Card><div className="row"><MessageCircle size={22} className="accent-text" /><h2 className="section-title">Добавим важные детали</h2></div><p className="muted text-small">{assist.provider === 'stub' ? 'Сейчас доступны шаблонные вопросы. Ответьте на подходящие вопросы, чтобы дополнить карточку задачи.' : 'Помощник учитывает описание, тему задачи и ваши предыдущие ответы. Добавляйте детали и запрашивайте следующий набор уточнений.'}</p><div className="stack">{assist.questions.map((question, index) => <div className="form-section" key={question.field}>
            <span className="eyebrow">ВОПРОС {index + 1} · {FIELD_LABELS[question.field]}</span>
            {assist.suggested_card[question.field].trim() && <div className="notice text-small"><div><strong>Уже известно</strong><p style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{question.field === 'topic' ? topicLabel(assist.suggested_card[question.field]) : assist.suggested_card[question.field]}</p></div></div>}
            <Textarea label={question.question} placeholder="Ваш ответ — или оставьте поле пустым" rows={3} value={answers[question.field] ?? ''} onChange={event => setAnswers(current => ({ ...current, [question.field]: event.target.value }))} disabled={!!busy} error={fieldErrors[question.field]} hint={question.field === 'title' || question.field === 'topic' ? 'Ответ заменит текущее значение этого поля.' : 'Ответ дополнит уже известные сведения.'} counter={`${candidate?.[question.field].length ?? 0} / ${fieldLimit(question.field)} символов в поле`} />
          </div>)}</div></Card>
          ...
          <div className="form-actions"><Button loading={busy === 'save'} disabled={!!busy || conflict || invalidCard} onClick={() => save()}>Применить и открыть карточку<ArrowRight size={17} /></Button><Button variant="secondary" loading={busy === 'assist'} disabled={!!busy || conflict || invalidCard} onClick={regenerate}><RefreshCw size={16} />Уточнить с учётом ответов</Button></div>
