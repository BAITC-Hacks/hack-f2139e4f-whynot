...
import { mergeTeamProfileDraft, teamProfileDraft } from '../ui/teamProfile';
...
export function TeamProfilePage() {
  const { actor, team, loading, teamLoading, teamError, refreshTeam } = useRole();
  const { toast } = useToast();
  const [profile, setProfile] = useState(() => teamProfileDraft(team));
  const previousProfile = useRef(teamProfileDraft(team));
  const { name, interests, skills, technologies } = profile;
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  const [errors, setErrors] = useState<Record<string, string>>({});
  useEffect(() => {
    const previous = previousProfile.current;
    const next = teamProfileDraft(team);
    setProfile(current => mergeTeamProfileDraft(current, previous, next));
